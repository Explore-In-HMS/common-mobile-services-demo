...
@AndroidEntryPoint
class PhoneSignInScreen : Fragment() {
    @Inject
    lateinit var authService: AuthService
    private lateinit var binding: FragmentPhoneSignInScreenBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentPhoneSignInScreenBinding.inflate(inflater)
        initUI()
        checkUser()
        return binding.root
    }
                ...
                .addOnSuccessListener { authUser ->
                    ...
                    with(binding) {
                        ...
                        binding.tvWelcomeUser.text =
                            getString(R.string.welcome_user_phone, authUser.phone)
                    }
                }
                .addOnFailureListener {
                    requireContext().toastShort(it.message.toString())
    ...
    private fun checkUser() {
        val userPhone = authService.getUser()?.phone
        if (userPhone != null) {
            binding.tvWelcomeUser.text = getString(R.string.welcome_user_phone, userPhone)
        } else {
